...
ErrorCode VideoDecoderFilter::PushData(const std::string& inPort, AVBufferPtr buffer, int64_t offset)
{
    if (state_ != FilterState::READY && state_ != FilterState::PAUSED && state_ != FilterState::RUNNING) {
        MEDIA_LOG_W("pushing data to decoder when state_ is %" PUBLIC_OUTPUT "d", static_cast<int>(state_.load()));
        return ErrorCode::ERROR_INVALID_OPERATION;
    }
    if (isFlushing_) {
        MEDIA_LOG_I("decoder is flushing, discarding this data from port %" PUBLIC_OUTPUT "s", inPort.c_str());
        return ErrorCode::SUCCESS;
    }
