                                ...
                                Plugin::TagMap& downstreamParams)
{
    ...
    MEDIA_LOG_E("select plugin %" PUBLIC_OUTPUT "s", selectedPluginInfo->name.c_str());
...
bool VideoSinkFilter::DoSync(int64_t pts) const
{
    ...
    if (delta > 0) {
        ...
        if (tempOut > 100) { // 100ms
            MEDIA_LOG_E("DoSync early %" PUBLIC_OUTPUT PRId64 " ms", tempOut);
        ...
        if (tempOut > 40) { // 40ms drop frame
            MEDIA_LOG_E("DoSync later %" PUBLIC_OUTPUT PRId64 " ms", tempOut);
            return false;
        }
    }
    return true;
...
void VideoSinkFilter::RenderFrame()
{
    ...
    if (INT64_MAX - latencyNano < oneBuffer->pts) {
        MEDIA_LOG_E("Video pts(%" PUBLIC_OUTPUT PRIu64 ") + latency overflow.", oneBuffer->pts);
        return;
    }
...
{
    MEDIA_LOG_D("video sink push data started, state_: %" PUBLIC_OUTPUT "d", state_.load());
    frameCnt_++;
...
ErrorCode VideoSinkFilter::Start()
{
    ...
    if (state_ != FilterState::READY && state_ != FilterState::PAUSED) {
        MEDIA_LOG_W("sink is not ready when start, state_: %" PUBLIC_OUTPUT "d", state_.load());
