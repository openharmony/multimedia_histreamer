...
bool AACAudioConfigParser::ExtractChannelElements(int& sceCnt, int& cpeCnt, int& lfeCnt, int& indepCce, int& depCce)
{
    ...
    switch (audioConfig_.channelConfig) {
        ...
        case 11:        // 11
            ...
            break;
        default:
            MEDIA_LOG_W("Unknown channel config in header: %" PUBLIC_OUTPUT "d", audioConfig_.channelConfig);
            return false;
    }
    return true;
}
...
#define AAC_PROFILE_VALUE_RANGE(maxChannelCnt, maxSampleRate, maxPcu, maxRcu)                                          \
    ...
    MEDIA_LOG_W("determined level: profile=%" PUBLIC_OUTPUT "u, sampleRate=%" PUBLIC_OUTPUT "u, channel_config=%"
                PUBLIC_OUTPUT "u, pcu=%" PUBLIC_OUTPUT "d,rcu=%" PUBLIC_OUTPUT "d",
...
bool AACAudioConfigParser::ParseProfile()
{
    ...
    switch (audioConfig_.audioObjectType) {
        ...
        default:
            audioConfig_.profile = AudioAacProfile::NONE;
            MEDIA_LOG_W("ParseProfile failed due to invalid profile index: %" PUBLIC_OUTPUT "u",
