...
do { \
    ...
    if (ret != OHOS::AudioStandard::SUCCESS) { \
        MEDIA_LOG_E(msg " failed return %" PUBLIC_OUTPUT "d", ret); \
...
Status AudioCapturePlugin::Prepare()
{
    ...
    if (size >= MAX_CAPTURE_BUFFER_SIZE) {
        MEDIA_LOG_E("bufferSize is too big: %" PUBLIC_OUTPUT "zu", size);
        return Status::ERROR_INVALID_PARAMETER;
    }
    bufferSize_ = size;
    MEDIA_LOG_D("bufferSize is: %" PUBLIC_OUTPUT "zu", bufferSize_);
    return Status::OK;
}
    ...
    switch (tag) {
        ...
        case Tag::AUDIO_CHANNELS: {
            if (params.audioChannel != capturerParams_.audioChannel) {
                MEDIA_LOG_W("audioChannel has changed from %" PUBLIC_OUTPUT "u to %" PUBLIC_OUTPUT "u",
        ...
        case Tag::AUDIO_SAMPLE_FORMAT: {
            if (params.audioSampleFormat != capturerParams_.audioSampleFormat) {
                MEDIA_LOG_W("audioSampleFormat has changed from %" PUBLIC_OUTPUT "u to %" PUBLIC_OUTPUT "u",
...
Status AudioCapturePlugin::GetSize(size_t& size)
{
    ...
    size = bufferSize_;
    MEDIA_LOG_D("bufferSize_: %" PUBLIC_OUTPUT "zu", size);
    return Status::OK;
}
