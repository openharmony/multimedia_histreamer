...
Status SdlVideoSinkPlugin::Init()
{
    ...
    if (SDL_Init(SDL_INIT_VIDEO)) {
        MEDIA_LOG_E("Init SDL fail: %" PUBLIC_OUTPUT "s", SDL_GetError());
...
Status SdlVideoSinkPlugin::CreateSdlDispContext()
{
    ...
    if (screen == nullptr) {
        MEDIA_LOG_E("Create window fail: %" PUBLIC_OUTPUT "s", SDL_GetError());
        return Status::ERROR_UNKNOWN;
    }
    ...
    if (renderer != nullptr) {
        if (!SDL_GetRendererInfo(renderer, &rendererInfo_)) {
            MEDIA_LOG_I("Init %" PUBLIC_OUTPUT "s renderer success", rendererInfo_.name);
        }
    }
    if ((renderer == nullptr) || (!rendererInfo_.num_texture_formats)) {
        MEDIA_LOG_E("Create renderer fail: %" PUBLIC_OUTPUT "s", SDL_GetError());
        return Status::ERROR_UNKNOWN;
    }
...
Status SdlVideoSinkPlugin::CreateSdlDispTexture()
{
    ...
    if (texture == nullptr) {
        MEDIA_LOG_E("Create texture fail: %" PUBLIC_OUTPUT "s", SDL_GetError());
        return Status::ERROR_UNKNOWN;
    }
...
Status SdlVideoSinkPlugin::VideoImageDisaplay(const std::shared_ptr<Buffer>& inputInfo)
{
    ...
    if (frameFormat != pixelFormat_) {
        MEDIA_LOG_I("pixel format change from %" PUBLIC_OUTPUT "u to %" PUBLIC_OUTPUT "u", pixelFormat_, frameFormat);
