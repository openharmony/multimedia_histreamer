...
Status Base::Init()
{
    MEDIA_LOG_D("%" PUBLIC_LOG "s Enter.", __FUNCTION__);
    ...
    if (pluginState_ != State::CREATED) {
        MEDIA_LOG_I("plugin %" PUBLIC_LOG "s has been already inited", plugin_->GetName().c_str());
        return Status::OK;
    }
...
Status Base::Deinit()
{
    ...
    if (pluginState_ == State::DESTROYED) {
        MEDIA_LOG_I("plugin %" PUBLIC_LOG "s already deinited, no need to destroy any more",
...
Status Base::Prepare()
{
    ...
    if (pluginState_ == State::PREPARED) {
        MEDIA_LOG_I("plugin %" PUBLIC_LOG "s already prepared, ignore this prepare", plugin_->GetName().c_str());
        return Status::OK;
    }
...
Status Base::Reset()
{
    ...
    if (pluginState_ == State::CREATED || pluginState_ == State::INITIALIZED) {
        MEDIA_LOG_I("plugin %" PUBLIC_LOG "s no need to reset in state %" PUBLIC_LOG "s",
...
Status Base::Start()
{
    ...
    if (pluginState_ == State::RUNNING) {
        MEDIA_LOG_I("plugin %" PUBLIC_LOG "s already in running state, ignore start", plugin_->GetName().c_str());
        return Status::OK;
    }
...
Status Base::Stop()
{
    ...
    if (pluginState_ != State::RUNNING && pluginState_ != State::PAUSED) {
        MEDIA_LOG_I("plugin %" PUBLIC_LOG "s not running or paused, no need to stop", plugin_->GetName().c_str());
        return Status::OK;
    }
