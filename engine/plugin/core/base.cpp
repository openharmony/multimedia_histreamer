...
Status Base::Init()
{
    ...
    if (pluginState_ != State::CREATED) {
        MEDIA_LOG_I("plugin %" PUBLIC_OUTPUT "s has been already inited", plugin_->GetName().c_str());
        return Status::OK;
    }
    ...
    MEDIA_LOG_D("%" PUBLIC_OUTPUT "s Exit.", __FUNCTION__);
    return ret;
}
...
Status Base::Prepare()
{
    ...
    if (pluginState_ == State::PREPARED) {
        MEDIA_LOG_I("plugin %" PUBLIC_OUTPUT "s already prepared, ignore this prepare", plugin_->GetName().c_str());
        return Status::OK;
    }
...
Status Base::Start()
{
    ...
    if (pluginState_ == State::RUNNING) {
        MEDIA_LOG_I("plugin %" PUBLIC_OUTPUT "s already in running state, ignore start", plugin_->GetName().c_str());
        return Status::OK;
    }
...
Status Base::Stop()
{
    ...
    if (pluginState_ != State::RUNNING && pluginState_ != State::PAUSED) {
        MEDIA_LOG_I("plugin %" PUBLIC_OUTPUT "s not running or paused, no need to stop", plugin_->GetName().c_str());
        return Status::OK;
    }
