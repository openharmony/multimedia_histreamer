...
Status Base::Init()
{
    ...
    if (pluginState_ != State::CREATED) {
        MEDIA_LOG_I("plugin %s has been already inited", plugin_->GetName().c_str());
        return Status::OK;
    }
...
Status Base::Prepare()
{
    ...
    if (pluginState_ == State::PREPARED) {
        MEDIA_LOG_I("plugin %s already prepared, ignore this prepare", plugin_->GetName().c_str());
        return Status::OK;
    }
...
Status Base::Reset()
{
    ...
    if (pluginState_ == State::CREATED || pluginState_ == State::INITIALIZED) {
        MEDIA_LOG_I("plugin %s no need to reset in state %s", plugin_->GetName().c_str(),
...
Status Base::Start()
{
    ...
    if (pluginState_ == State::RUNNING) {
        MEDIA_LOG_I("plugin %s already in running state, ignore start", plugin_->GetName().c_str());
        return Status::OK;
    }
