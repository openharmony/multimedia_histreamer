...
#define PROFILE_BEGIN(message, ...)                                                                                    \
    do {                                                                                                               \
        MEDIA_LOG_W(message ", timestamp(ms): %" PUBLIC_OUTPUT PRId64, ##__VA_ARGS__, SteadyClock::GetCurrentTimeMs());              \
    } while (0);                                                                                                       \
...
#define PROFILE_END(message, ...)                                                                                      \
    do {                                                                                                               \
        MEDIA_LOG_W(message ", timestamp(ms): %" PUBLIC_OUTPUT PRId64 ", duration(ms): %" PUBLIC_OUTPUT PRId64, ##__VA_ARGS__,                     \
