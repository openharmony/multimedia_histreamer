...
HWTEST(TestPluginSetting, set_para_to_sink_plugins, TestSize.Level1)
{
    std::shared_ptr<Ffmpeg::ResamplePara> resamplePara = std::make_shared<Ffmpeg::ResamplePara>();
    resamplePara->channels = 1;
...
HWTEST(TestPluginSetting, find_unavailable_plugins, TestSize.Level1)
{
    Meta meta;
    std::string artist("unit test");
    meta.Set<Tag::MEDIA_TITLE>(artist);
    auto capability = MetaToCapability(meta);
    auto plugins = FindAvailablePlugins(*capability, Plugin::PluginType::AUDIO_DECODER);
    ASSERT_TRUE(plugins.size() == 0);
}

HWTEST(TestPluginSetting, hdf_status_to_string, TestSize.Level1)
{
...
HWTEST(TestPluginSetting, omx_error_type_to_string, TestSize.Level1)
{
...
HWTEST(TestPluginSetting, translate_to_omx_flag_set, TestSize.Level1)
{
...
HWTEST(TestPluginSetting, transfor_hdi_ret_value_to_status, TestSize.Level1)
{
...
HWTEST(TestPluginSetting, omx_state_to_string, TestSize.Level1)
{
...
HWTEST(TestPluginSetting, coding_type_hst_to_hdi, TestSize.Level1)
{
...
HWTEST(TestPluginSetting, format_hst_to_omx, TestSize.Level1)
{
...
HWTEST(TestPluginSetting, get_omx_buffer_type, TestSize.Level1)
{
...
HWTEST(TestPluginSetting, assign_parameter_if_match, TestSize.Level1)
{
    uint32_t value = 0;
    uint32_t ret;
...
HWTEST(TestPluginSetting, codec_buffer_pool, TestSize.Level1)
{
    PluginParaAllowedMap allowedMap = PluginParameterTable::FindAllowedParameterMap(FilterType::MEDIA_SOURCE);
    ASSERT_TRUE(allowedMap.size() == 0);
    allowedMap = PluginParameterTable::FindAllowedParameterMap(FilterType::AUDIO_DECODER);
    ASSERT_TRUE(allowedMap.size() > 0);
    allowedMap = PluginParameterTable::FindAllowedParameterMap(FilterType::VIDEO_DECODER);
    ASSERT_TRUE(allowedMap.size() > 0);
    allowedMap = PluginParameterTable::FindAllowedParameterMap(FilterType::AUDIO_ENCODER);
    ASSERT_TRUE(allowedMap.size() > 0);
    allowedMap = PluginParameterTable::FindAllowedParameterMap(FilterType::VIDEO_ENCODER);
    ASSERT_TRUE(allowedMap.size() > 0);
    allowedMap = PluginParameterTable::FindAllowedParameterMap(FilterType::CAPTURE_SOURCE);
    ASSERT_TRUE(allowedMap.size() == 0);
    allowedMap = PluginParameterTable::FindAllowedParameterMap(FilterType::DEMUXER);
    ASSERT_TRUE(allowedMap.size() == 0);
    allowedMap = PluginParameterTable::FindAllowedParameterMap(FilterType::MUXER);
    ASSERT_TRUE(allowedMap.size() > 0);
    allowedMap = PluginParameterTable::FindAllowedParameterMap(FilterType::AUDIO_SINK);
    ASSERT_TRUE(allowedMap.size() > 0);
    allowedMap = PluginParameterTable::FindAllowedParameterMap(FilterType::VIDEO_SINK);
    ASSERT_TRUE(allowedMap.size() > 0);
    allowedMap = PluginParameterTable::FindAllowedParameterMap(FilterType::OUTPUT_SINK);
    ASSERT_TRUE(allowedMap.size() == 0);
    allowedMap = PluginParameterTable::FindAllowedParameterMap(FilterType::NONE);
    ASSERT_TRUE(allowedMap.size() == 0);
}

HWTEST(TestPluginSetting, translate_plugin_status, TestSize.Level1)
{
...
HWTEST(TestPluginSetting, find_allowed_parameter_map, TestSize.Level1)
{
    FilterType filterType = FilterType::NONE;
    PluginParaAllowedMap allowedMap = PluginParameterTable::FindAllowedParameterMap(filterType);
    ASSERT_TRUE(allowedMap.size() == 0);
    filterType = FilterType::AUDIO_DECODER;
    allowedMap = PluginParameterTable::FindAllowedParameterMap(filterType);
    ASSERT_TRUE(allowedMap.size() != 0);
}
