...
*/
struct Allocator {
    explicit Allocator(MemoryType type = MemoryType::VIRTUAL_MEMORY) : memoryType(type) {}
...
public:
    ...
    virtual size_t Write(const uint8_t* in, size_t writeSize, size_t position = MEM_INVALID_POSITION);

    virtual size_t Read(uint8_t* out, size_t readSize, size_t position = MEM_INVALID_POSITION);
